...
locals {
  ...
  graviton_ng = {
    ...
    instance_types = local.graviton_node_size["default"]
    k8s_labels = {
      Terraform                                   = "true"
      "cloud-platform.justice.gov.uk/graviton-ng" = "true"
      Cluster                                     = terraform.workspace
      Domain                                      = local.fqdn
    }
    additional_tags = {
...
module "aws_eks_addons" {
  source                  = "github.com/ministryofjustice/cloud-platform-terraform-eks-add-ons?ref=arm-fb"

locals {
  ...
  subnets = { for idx, subnet in var.firewall_subnets : "subnet${idx + 1}" =>
    {
      subnet_id       = subnet
      ip_address_type = "IPV4"
    }
  }
  vpc = var.firewall_vpc
}
...
module "cloud-platform-firewall-rule-group" {
  ...
  rule_group = {
    ...
    rule_variables = {
      ip_sets = [
        { key    = "HOME_NET"
          ip_set = { definition = var.firewall_home_net }
        },
        { key    = "EXTERNAL_NET"
          ip_set = { definition = var.firewall_external_net }
        }
      ]
    }
  }
...
resource "aws_cloudwatch_log_group" "logs" {
  name_prefix       = var.firewall_log_group_prefix
  retention_in_days = 7
}
provider "elasticsearch" {
  alias               = "app_logs_live_2"
  url                 = "https://${aws_opensearch_domain.live_2_app_logs.endpoint}"
  aws_assume_role_arn = aws_iam_role.os_access_role_live_2_app_logs.arn
  aws_profile         = "moj-cp"
...
locals {
  live_2_app_logs_domain = "cp-live-2-app-logs"
  live_2_app_logs_tags = {
    Domain = local.live_2_app_logs_domain
  }
}

data "aws_eks_node_groups" "live_2" {
  cluster_name = "live-2" # change to the cluster you need -- note there is no terraform.workspace at the account level
}

data "aws_eks_node_group" "live_2" {
  for_each = data.aws_eks_node_groups.live_2.names
...
data "aws_iam_policy_document" "assume_role_policy_live_2_app_logs" {
  statement {
    actions = [
      "sts:AssumeRole",
    ]
    ...
    Statement = [
      {
        ...
        Effect = "Allow"
        Resource = [
          "arn:aws:es:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:domain/${local.live_2_app_logs_domain}/*",
        ]
      },
    ]
  })
...
data "aws_iam_policy_document" "live_2_app_logs" {
  statement {
    actions = [
      "es:Describe*",
      "es:List*",
...
data "aws_route53_zone" "live_2_cloud_platform_justice_gov_uk" {
  name = "live-2.cloud-platform.service.justice.gov.uk."
}
...
resource "aws_opensearch_domain" "live_2_app_logs" {
  ...
  advanced_security_options {
    ...
    master_user_options {
      master_user_arn = aws_iam_role.os_access_role_live_2_app_logs.arn
    }
  }
  ...
  domain_endpoint_options {
    ...
    custom_endpoint_certificate_arn = module.acm_live_2_app_logs.acm_certificate_arn
    custom_endpoint                 = "app-logs.${data.aws_route53_zone.live_2_cloud_platform_justice_gov_uk.name}"
  }
  ...
  encrypt_at_rest {
    enabled    = true
    kms_key_id = aws_kms_key.live_2_app_logs.key_id
  }
...
resource "aws_route53_record" "opensearch_custom_domain_live_2_app_logs" {
  zone_id = data.aws_route53_zone.live_2_cloud_platform_justice_gov_uk.zone_id
  name    = "app-logs"
  type    = "CNAME"
  ttl     = 600 # 10 mins

  records = [aws_opensearch_domain.live_2_app_logs.endpoint]
}

resource "aws_opensearch_domain_policy" "live_2_app_logs" {
  domain_name     = aws_opensearch_domain.live_2_app_logs.domain_name
  access_policies = data.aws_iam_policy_document.live_2_app_logs.json
}

resource "elasticsearch_opensearch_ism_policy" "ism_policy_live_2_app_logs" {
  provider  = elasticsearch.app_logs_live_2
  ...
  body = templatefile("${path.module}/resources/opensearch/ism-policy.json.tpl", {
    ...
    index_pattern     = jsonencode(var.index_pattern_live_2_app_logs)
  })

  depends_on = [
    aws_opensearch_domain_saml_options.live_2_app_logs,
  ]
}

resource "aws_opensearch_domain_saml_options" "live_2_app_logs" {
  domain_name = aws_opensearch_domain.live_2_app_logs.domain_name
  saml_options {
    enabled = true
    ...
    idp {
      ...
      metadata_content = data.http.saml_metadata_live_2_app_logs.response_body
    }

    master_backend_role = aws_iam_role.os_access_role_live_2_app_logs.arn
    master_user_name    = aws_iam_role.os_access_role_live_2_app_logs.arn
...
resource "elasticsearch_opensearch_roles_mapping" "all_access_live_2_app_logs" {
  provider    = elasticsearch.app_logs_live_2
  ...
  backend_roles = concat([
    ...
    aws_iam_role.os_access_role_live_2_app_logs.arn,
  ], values(data.aws_eks_node_group.current)[*].node_role_arn, values(data.aws_eks_node_group.live_2)[*].node_role_arn)
...
resource "elasticsearch_opensearch_roles_mapping" "security_manager_live_2_app_logs" {
  provider    = elasticsearch.app_logs_live_2
...
resource "elasticsearch_opensearch_role" "webops_live_2_app_logs" {
  provider    = elasticsearch.app_logs_live_2
  role_name   = "webops"
...
resource "elasticsearch_opensearch_roles_mapping" "webops_live_2_app_logs" {
  provider      = elasticsearch.app_logs_live_2
  role_name     = "webops"
  ...
  depends_on = [
    aws_opensearch_domain_saml_options.live_2_app_logs,
    elasticsearch_opensearch_role.webops_live_2_app_logs
  ]
}

resource "elasticsearch_opensearch_role" "org_sec_audit_live_2_app_logs" {
  provider    = elasticsearch.app_logs_live_2
...
resource "elasticsearch_opensearch_roles_mapping" "org_sec_audit_live_2_app_logs" {
  provider      = elasticsearch.app_logs_live_2
  role_name     = "organisation-security-auditor"
  description   = "Mapping AWS IAM roles to ES role webops"
  ...
  depends_on = [
    aws_opensearch_domain_saml_options.live_2_app_logs,
    elasticsearch_opensearch_role.org_sec_audit_live_2_app_logs
  ]
}

resource "elasticsearch_opensearch_role" "all_org_members_live_2_app_logs" {
  provider    = elasticsearch.app_logs_live_2
...
resource "elasticsearch_opensearch_roles_mapping" "all_org_members_live_2_app_logs" {
  provider      = elasticsearch.app_logs_live_2
  role_name     = "all_org_members"
  ...
  depends_on = [
    aws_opensearch_domain_saml_options.live_2_app_logs,
    elasticsearch_opensearch_role.all_org_members_live_2_app_logs
  ]
}

resource "elasticsearch_opensearch_roles_mapping" "opensearch_dashboards_user_live_2_app_logs" {
  provider      = elasticsearch.app_logs_live_2
  ...
  depends_on = [
    aws_opensearch_domain_saml_options.live_2_app_logs,
    elasticsearch_opensearch_role.all_org_members_live_2_app_logs,
    elasticsearch_opensearch_roles_mapping.all_org_members_live_2_app_logs
  ]
}

resource "elasticsearch_opensearch_roles_mapping" "opensearch_reporting_live_2" {
  provider      = elasticsearch.app_logs_live_2
  role_name     = "reports_instances_read_access"
  description   = "Mapping all_org_members to the reporting plugin"
  backend_roles = ["all-org-members"]
  depends_on = [
    aws_opensearch_domain_saml_options.live_2_app_logs,
    elasticsearch_opensearch_role.all_org_members_live_2_app_logs,
...
data "http" "saml_metadata_live_2_app_logs" {
  url    = "https://${var.auth0_tenant_domain}/samlp/metadata/${auth0_client.opensearch_live_2_app_logs.client_id}"
  method = "GET"
}
...
resource "auth0_client" "opensearch_live_2_app_logs" {
  ...
  callbacks = ["https://${aws_route53_record.opensearch_custom_domain_live_2_app_logs.fqdn}/_dashboards/_opendistro/_security/saml/acs"]
  logo_uri  = "https://ministryofjustice.github.io/assets/moj-crest.png"
  addons {
    samlp {
      audience    = "https://${aws_route53_record.opensearch_custom_domain_live_2_app_logs.fqdn}"
      destination = "https://${aws_route53_record.opensearch_custom_domain_live_2_app_logs.fqdn}/_dashboards/_opendistro/_security/saml/acs"
      mappings = {
...
module "live_2_app_logs_opensearch_monitoring" {
  source                                   = "github.com/ministryofjustice/cloud-platform-terraform-opensearch-cloudwatch-alarm?ref=0.0.2"
  alarm_name_prefix                        = "CP-live-2-app-logs-"
  domain_name                              = local.live_2_app_logs_domain
  sns_topic                                = module.baselines.slack_sns_topic
  min_available_nodes                      = aws_opensearch_domain.live_2_app_logs.cluster_config[0].instance_count
  ...
  free_storage_space_threshold = aws_opensearch_domain.live_2_app_logs.ebs_options[0].volume_size * 0.25 * 1024
  ...
  free_storage_space_total_threshold = aws_opensearch_domain.live_2_app_logs.ebs_options[0].volume_size * aws_opensearch_domain.live_2_app_logs.cluster_config[0].instance_count * 0.25 * 1024
  tags                               = local.live_2_app_logs_tags
}

...
module "iam" {
  source = "github.com/ministryofjustice/cloud-platform-terraform-awsaccounts-iam?ref=0.0.33"
...
module "s3_bucket_velero" {
  source  = "terraform-aws-modules/s3-bucket/aws"
  version = "4.2.0" # 2.15 pins to terraform-aws-provider <= 4, so this module needs to be upgraded to 3.6.0, which has some resource changes (which may require manual intervention for terraform state imports)

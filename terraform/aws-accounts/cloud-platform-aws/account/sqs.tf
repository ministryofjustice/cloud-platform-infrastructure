...
# SQS Queue to present the RDS logs bucket updates
resource "aws_sqs_queue" "cp_rds_logs_queue" {
  name                       = "cp_rds_logs_queue"
  sqs_managed_sse_enabled    = true   # Using managed encryption
  delay_seconds              = 0      # The default is 0 but can be up to 15 minutes
  max_message_size           = 262144 # 256k which is the max size
  message_retention_seconds  = 345600 # This is 4 days. The max is 14 days
...
# This policy grants queue send message from the RDS logs bucket
resource "aws_sqs_queue_policy" "rds_logs_queue_policy" {
  queue_url = aws_sqs_queue.cp_rds_logs_queue.id
  policy    = data.aws_iam_policy_document.rds_logs_queue_policy_document.json
}

data "aws_iam_policy_document" "rds_logs_queue_policy_document" {
  statement {
    sid    = "AllowSendMessage"
    effect = "Allow"
    principals {
    ...
    condition {
      test     = "ArnEquals"
      variable = "aws:SourceArn"
      values   = [module.rds_logs_to_cortex_s3.bucket_arn]
    }
  }
}

# S3 bucket event notification for updates from the RDS logs bucket
resource "aws_s3_bucket_notification" "rds_logs_bucket_notification" {
  bucket = module.rds_logs_to_cortex_s3.bucket_name
  queue {
    queue_arn = aws_sqs_queue.cp_rds_logs_queue.arn
    events    = ["s3:ObjectCreated:*"] # Events to trigger the notification
  }
}

##### IAM User & Resources to access the sqs queue and read cloudtrail, flowlogs, Route53 Resolver query, application, cloudfront, modsec and RDS log buckets

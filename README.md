...
1. The pipeline: The pipeline is based on AWS CodePipeline/Codebuild. It is triggered automatically after a change is promoted to master. The pipeline can be managed with the terraform templates that live in cluster-pipeline/terraform directory. The pipeline will execute what ever is defined in cluster-pipeline/buildspec.yaml

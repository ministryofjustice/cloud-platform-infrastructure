...
try:
  workspaces = subprocess.check_output(['terraform', 'workspace', 'list']).decode('utf-8').split()
  workspaces.remove('*')
except subprocess.CalledProcessError as e:
  print('error listing workspaces, exit.')

try:
  yaml = [ s.strip('.yaml') for s in subprocess.check_output(['ls', '../../kops/']).decode('utf-8').split() ]
except subprocess.CalledProcessError as e:
  print('error listing cluster yaml files, exit.')

# query the kops state store to list the active clusters
try:
  clusters = json.loads(str(subprocess.check_output(['kops', 'get', 'clusters', '--state=s3://' + statestore, '--output=json' ]).decode('utf-8')))
except subprocess.CalledProcessError as e:

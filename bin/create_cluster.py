...
#  p.pprint(policies)
  ...
  template['spec'].update({'topology': { 'bastion': { 'bastionPublicName': topology }, 'dns': { 'type': 'Public' }, 'masters': 'private', 'nodes': 'private' } } )

  subnets = []
  if len(internal_subnets) == len(external_subnets) == len(availability_zones):
    for i in range(len(external_subnets)):
      subnets.append( { 'cidr': external_subnets[i], 'name': availability_zones[i], 'type': 'Private', 'zone': availability_zones[i] } )
    for i in range(len(internal_subnets)):
      subnets.append( { 'cidr': internal_subnets[i], 'name': 'utility-' + availability_zones[i], 'type': 'Utility', 'zone': availability_zones[i] } )

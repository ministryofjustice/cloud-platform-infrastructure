...
get_cold_size () {
    ...
    if [[ $CAUSE =~ "because another index with the same name already exists in cold storage" ]]; then
        ...
        if ((COLD_INDEX_SIZE < WARM_INDEX_SIZE)); then
            ...
            curl -X DELETE -L --user $AWS_ACCESS_KEY_ID:$AWS_SECRET_ACCESS_KEY "${PIPELINE_AWS_SESSION_TOKEN}" --aws-sigv4 "aws:amz:eu-west-2:es" https://app-logs.cloud-platform.service.justice.gov.uk/_cold/$INDEX_NAME

            sleep 0.2

            curl -X POST -L --user $AWS_ACCESS_KEY_ID:$AWS_SECRET_ACCESS_KEY "${PIPELINE_AWS_SESSION_TOKEN}" --aws-sigv4 "aws:amz:eu-west-2:es" https://app-logs.cloud-platform.service.justice.gov.uk/_plugins/_ism/retry/$INDEX_NAME

        else
            echo "warm index is tiny $WARM_INDEX_SIZE $COLD_INDEX_SIZE $INDEX_NAME $CAUSE"
            curl -X DELETE -L --user $AWS_ACCESS_KEY_ID:$AWS_SECRET_ACCESS_KEY "${PIPELINE_AWS_SESSION_TOKEN}" --aws-sigv4 "aws:amz:eu-west-2:es" https://app-logs.cloud-platform.service.justice.gov.uk/$INDEX_NAME
...
curl -L --user $AWS_ACCESS_KEY_ID:$AWS_SECRET_ACCESS_KEY "${PIPELINE_AWS_SESSION_TOKEN}" --aws-sigv4 "aws:amz:eu-west-2:es" "https://app-logs.cloud-platform.service.justice.gov.uk/_cat/indices/_warm?v" | awk '{ print $3 " " $7 }' > all_warm_indices
